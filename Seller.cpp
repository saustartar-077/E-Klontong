    ...
    void muatProdukDariCSV() {
        ...
        if (!file.is_open()) {
            return; // Kalau belum ada file, abaikan
        }

        string header;
        getline(file, header); // Lewati baris header

        jumlahProduk = 0;
        while (!file.eof() && jumlahProduk < MAX_PRODUK) {
            Produk p;
            char koma; // untuk baca koma
            file >> p.id >> koma;
            getline(file, p.nama, ',');
            file >> p.stok >> koma >> p.harga;
            if (!file.fail()) {
    ...
    void tambahProduk(int id, string nama, int stok, int harga) {
        daftarProduk[jumlahProduk++] = {id, nama, stok, harga};
        simpanProdukKeCSV();
    }

    void tampilkanProduk() {
        if (jumlahProduk == 0) {  // Cek jika tidak ada produk
        ...
        for (int i = 0; i < jumlahProduk; i++) {
            cout << "[" << daftarProduk[i].id << "] " << daftarProduk[i].nama
                ...
                << " | Harga: Rp " << fixed << setprecision(0) << daftarProduk[i].harga << endl;
        }
    }
    ...
    int prosesTransaksi() {
        ...
        NodeKeranjang* temp = headKeranjang;
        while (temp != nullptr) {
            int subTotal = temp->jumlah * temp->produk->harga;
            total += subTotal;
